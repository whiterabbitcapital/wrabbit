		...
		<tr>
			...
			<td>
				...
				@else
					...
					@if (data_get($transaction, 'meta.account') && data_get($transaction, 'meta.account') == data_get($transaction, 'pay_to'))
					<div class="label lead-text">{{ __("Account No / ID") }}</div>
					<div class="data mb-1">{{ data_get($transaction, 'meta.account') }}</div>
					@endif
